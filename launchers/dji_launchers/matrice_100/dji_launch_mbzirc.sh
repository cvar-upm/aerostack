...
if [ -z $DEBUG_MODE ]
	then
		...
		tmux send-keys "roslaunch driverDjiMatrice210ROSModule driverDJIMatrice210ROSModule.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK}" C-m
		...
		tmux new-window -t $SESSION:5 -n 'droneRobotLocalizationROSModule'
...
roslaunch driverDjiMatrice210ROSModule driverDJIMatrice210ROSModule.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK};
		...
		roslaunch ${AEROSTACK_STACK}/launchers/dji_launchers/matrice_100/launch_files/lightware_altitude_node.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK} serial_port:="/dev/ttyUSB0";	
					 exec bash\""  \
