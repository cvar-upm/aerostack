function [drone_data] = eventlog_readdata( file_path, varargin)
% % eventlog_readdata:
...
import log_tools_lib.log_reading_comptags;
...
disp(['time_init:',num2str(time_init)]);
disp(['time_end:',num2str(time_end)]);
...
for i=1:length(tags)
    ...
    domain_name = strrep(domain_name, ']', '');
    domain_name = strrep(domain_name, ';', '.');
    time_name = ['drone_data.',domain_name, '.', 'time'];
...
%         eval([time_name,' = ',time_name,' - ',time_name,'(1);']);
        time_init_name = ['drone_data.',domain_name, '.', 'time_init'];
        time_end_name = ['drone_data.',domain_name, '.', 'time_end'];
        eval([time_init_name,' = ',num2str(time_init),';']);
        eval([time_end_name,' = ',num2str(time_end),';']);
    end
    for j=3:2:length(data{aux_index(i)})
        var_name  = ['drone_data.',domain_name, '.', data{aux_index(i)}{j}];
