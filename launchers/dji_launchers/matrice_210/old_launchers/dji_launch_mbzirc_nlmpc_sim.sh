...
if [ -z $DEBUG_MODE ]
	then
		...
		tmux new-window -t $SESSION:8 -n 'NonLinearMPC'
	    	tmux send-keys "roslaunch mav_nonlinear_mpc mav_nonlinear_mpc.launch mav_name:=m210_sim --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE" C-m
