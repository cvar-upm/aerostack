...
roslaunch ${AEROSTACK_STACK}/launchers/pixhawk_launchers/launch_files/Ekf_new.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK};
						exec bash\""  \
...
roslaunch ${AEROSTACK_STACK}/launchers/pixhawk_launchers/launch_files/hector_slam_new.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK};
						exec bash\""  \
	--tab --title "Hector Slam" --command "bash -c \"
roslaunch hector_slam_ros_module hectorSlamROSModule.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK};
						exec bash\""  \
	--tab --title "Laser Based Pose Estimation Module" --command "bash -c \"
roslaunch laser_based_pose_estimation droneLaserBasedPoseEstimationROSModule.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK};
						exec bash\""  \
