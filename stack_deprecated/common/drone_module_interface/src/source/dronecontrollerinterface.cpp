#include "dronecontrollerinterface.h"

DroneControllerInterface::DroneControllerInterface(const std::string &module_name_str_in, const ModuleNames::name module_name_enum_in) :
...
DroneControllerInterface::~DroneControllerInterface() {

}

void DroneControllerInterface::open(ros::NodeHandle & nIn) {
    DroneModuleInterface::open(nIn);

    // Subscribers
    controlModeSub=n.subscribe(std::string(MODULE_NAME_CONTROLLER)+"/controlMode", 1, &DroneControllerInterface::controlModeSubCallback, this);
    drone_position_reference_subscriber   = n.subscribe(DRONE_TRAJECTORY_CONTROLLER_INTERFACE_POSITION_REF_REBROADCAST_SUBSCRIPTION, 1, &DroneControllerInterface::droneCurrentPositionRefsSubCallback, this);
    drone_speed_reference_subscriber      = n.subscribe(DRONE_TRAJECTORY_CONTROLLER_INTERFACE_SPEED_REF_REBROADCAST_SUBSCRIPTION, 1, &DroneControllerInterface::droneCurrentSpeedsRefsSubCallback, this);
    drone_trajectory_reference_subscriber = n.subscribe(DRONE_TRAJECTORY_CONTROLLER_INTERFACE_TRAJECTORY_REF_REBROADCAST_SUBSCRIPTION, 1, &DroneControllerInterface::droneCurrentTrajectoryRefCommandCallback, this);
    // Publishers
    ...
    // Services
    setControlModeClientSrv=n.serviceClient<droneMsgsROS::setControlMode>(std::string(MODULE_NAME_CONTROLLER)+"/setControlMode");
}

void DroneControllerInterface::controlModeSubCallback(const droneMsgsROS::droneTrajectoryControllerControlMode::ConstPtr &msg) {
    switch (msg->command) {
    ...
    case Controller_MidLevel_controlMode::POSITION_TO_COMMAND_CONTROL:
        last_received_control_mode = Controller_MidLevel_controlMode::POSITION_TO_COMMAND_CONTROL;
        break;
...
bool DroneControllerInterface::setControlMode(Controller_MidLevel_controlMode::controlMode new_control_mode) {
...
void DroneControllerInterface::droneCurrentPositionRefsSubCallback(const droneMsgsROS::dronePose::ConstPtr &msg) {
    current_drone_position_reference_ = (*msg);
}

void DroneControllerInterface::droneCurrentSpeedsRefsSubCallback(const droneMsgsROS::droneSpeeds::ConstPtr &msg) {
    current_drone_speed_reference_    = (*msg);
}

void DroneControllerInterface::droneCurrentTrajectoryRefCommandCallback(const droneMsgsROS::dronePositionTrajectoryRefCommand::ConstPtr& msg) {
    current_drone_trajectory_command_ = (*msg);
}


void DroneControllerInterface::publishDronePositionReference(double x, double y, double z) {
...
void DroneControllerInterface::publishDroneYawReference(double yaw)
{
    if (yaw > M_PI){
        yaw -= 2 * M_PI;
    }
...
void DroneControllerInterface::publishDroneSpeedsReference(double vxfi, double vyfi) {
...
void DroneControllerInterface::publishDroneRelSpeedsReference(double vlx, double vly, double drone_yaw_rad)
{
    double vxfi, vyfi;
...
void DroneControllerInterface::publishDroneAbsTrajecotryReference(const std::vector<SimpleTrajectoryWaypoint> * const trajectory_waypoints_out, const int initial_checkpoint_out, const bool is_periodic_out ) {
...
void DroneControllerInterface::publishDroneRelTrajecotryReference( const std::vector<SimpleTrajectoryWaypoint> * const trajectory_waypoints_out, const int initial_checkpoint_out, const bool is_periodic_out)
{
    droneMsgsROS::dronePositionTrajectoryRefCommand drone_trajectory_reference_command;
    //    drone_trajectory_reference_command.target_frame = "drone_GMR";
...
std::stringstream *DroneControllerInterface::getControllerState()
{
    ...
    case Controller_MidLevel_controlMode::POSITION_CONTROL:
        ...
        break;
    case Controller_MidLevel_controlMode::POSITION_TO_COMMAND_CONTROL:
        interface_printout_stream << "POSITION_TO_COMMAND" << std::endl;
        break;
    case Controller_MidLevel_controlMode::TRAJECTORY_CONTROL:
...
std::stringstream *DroneControllerInterface::getPositionReferences_GMRwrtGFF_Stream()
{
    interface_printout_stream.clear();
    interface_printout_stream.str(std::string());
    interface_printout_stream
...
std::stringstream *DroneControllerInterface::getSpeedReferences_GMRwrtGFF_Stream()
{
    interface_printout_stream.clear();
    interface_printout_stream.str(std::string());
    interface_printout_stream
