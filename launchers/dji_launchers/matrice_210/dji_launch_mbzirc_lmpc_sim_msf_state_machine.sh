	...
	then
		...
		tmux new-window -t $SESSION:8 -n 'LinearMPC'
	    	tmux send-keys "roslaunch mav_linear_mpc mav_linear_mpc_msf.launch mav_name:=m210_sim_msf --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE" C-m
                ...
                tmux new-window -t $SESSION:9 -n 'State Machine'
                tmux send-keys "roslaunch mission_controller_mbzirc_process mission_controller_mbzirc_process.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE" C-m
