...
roslaunch droneSubMissionPlannerROSModule droneSubMissionPlannerROSModule.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${DRONE_STACK} drone_Estimated_Pose_Topic_Name:=ArucoSlam_EstimatedPose;
						exec bash\"" &
