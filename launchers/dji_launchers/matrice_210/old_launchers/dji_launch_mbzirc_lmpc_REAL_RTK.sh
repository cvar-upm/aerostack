...
if [ -z $DEBUG_MODE ]
	then
		...
		tmux new-window -t $SESSION:5 -n 'Robot localization'
                tmux send-keys "roslaunch ${AEROSTACK_STACK}/launchers/dji_launchers/matrice_210/launch_files/EKF_RTK.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${AEROSTACK_STACK}" C-m
